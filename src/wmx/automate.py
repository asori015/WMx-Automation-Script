...
def initWMx() -> http.client.HTTPConnection:
    ...
    print('what is this')
    exit()
    params = '{"UserId":"","Psw":"","SiteId":"ONT005"}'
...
def x():
    ...
    def changeText():
        x.configure(text="Welcome to StackHowTo!") 

    window = tk.Tk()
    window.title("WMX Automation Tool")
    ...
    btn_open = tk.Button(frm_buttons, text="Open Excel File", command=open_file)
    btn_save = tk.Button(frm_buttons, text="Save As...", command=save_file)
    btn_change = tk.Button(frm_buttons, text="Change the text", command=changeText)
    ...
    x = tk.Label(text="Hello, Tkinter", background='red')
    x.grid(row=0, column=1, sticky="nw")
